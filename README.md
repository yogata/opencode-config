...
| スキル | 役割 |
|--------|------|
| `issue-guide-phases` | フェーズ定義・SSoT遷移・パターン判定基準を提供 |
| `issue-guide-reports` | 完了報告フォーマットとチェックボックス更新ルールを提供 |
| `issue-guide-review` | レビューNG時の対応フローと次コマンド推論ルールを提供 |
...
| `deviation-check` | 実装と要件の乖離を検出する品質ゲート |
...
| deviation-check | 実装と要件の乖離を検出する品質ゲート手法 |
...
| archive | 完了したPlanのアーカイブ。`.sisyphus/archive/` に移動 |
