...
| アーティファクト | 役割 | 格納先 |
| ---------------- | ---- | ------ |
| REQ（要件doc） | ユーザー視点の機能要件 | `docs/requirements/REQ-NNNN-slug.md` |
| コード | 実装そのもの | ソースコード |
| テスト | 振る舞い仕様 | テストファイル |
...
✅ パターン{X}（{規模}）と判定しました。
  Issue状態: {フェーズ}
  {パターンBの場合: REQファイル: {CREATE/APPEND/UPDATE}: REQ-{NNNN}-{slug}}
...
✅ Issue #{N} を作成しました（パターン{X}）。
  {パターンBの場合: docs/requirements/REQ-{NNNN}-{slug}.md を作成しました。}
...
🎉 フロー完了しました。
  ...
  - Planファイルアーカイブ完了
  {パターンBの場合: - docs/requirements/REQ-{NNNN}-{slug}.md 作成済み, docs/specs/ 更新済み}
```
...
| requirements/ | 要件管理 | README.md + REQ-NNNN-slug.md | issue-req(CREATE), issue-create(READ), issue-update(UPDATE) |
