---
name: issue-workflow
description: issue-req、issue-create、issue-work、issue-update、issue-closeコマンドから明示的に呼び出される。パターン判定、状態判定、完了報告生成、次のステップ提案、エラーハンドリングを担当。gh issueをSSoT（信頼できる唯一の情報源）として使用。
---
...
- **SSoT**: GitHub Issue（`gh issue`）を信頼できる唯一の情報源とする
