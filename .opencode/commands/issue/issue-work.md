...
- **11a: ローカル検証**（型チェック・Lint・ビルド・テスト）
  - 検証失敗時: エラー内容を報告し、実装フェーズ（Step 6）へ戻ることを提案（ユーザー判断）
  - 検証成功時のみ11bへ進む
...
- **11c: デプロイ検証**（`gh pr checks` によるCI/CDステータス確認）
  ...
  - **ポーリング動作**:
    ...
    - ビルド失敗（failed）: エラー原因を特定し、実装フェーズ（Step 6）へループバック
  ...
  - **ループバック時の動作**: 実装フェーズ（Step 6）に戻り、CI失敗原因に基づく修正を実施後、再度11a〜11cを実行
...
| 提出フェーズ Step 11c デプロイ検証失敗 | 実装フェーズのStep 6 | CI失敗原因に基づく修正後、11a〜11cを再実行（最大3回） |
...
- 自動リトライは同一入力・同一操作の機械的再試行のみ許可する（例: ネットワークタイムアウト時のghコマンド再実行）。判断を伴う修正（コード変更、要件解釈の変更等）は禁止。必ず停止してユーザーに判断を委ねること
