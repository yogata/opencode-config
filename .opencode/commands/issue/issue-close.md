 ...
 4. PRマージ（`gh pr merge`）→ 対応記録をIssueにコメント追記 → テンプレート: `.opencode/commands/issue/templates/issue_comment_feature_implementation.md`（パターンB）または `.opencode/commands/issue/templates/issue_comment_bug_record.md`（パターンA/C/D）を Read tool で読み込む
  ...
  5. Issue本文のテスト戦略チェックボックス更新:
    ...
    - `gh-cli-best-practices` に従い `--body-file` で `gh issue edit` を実行してIssue本文を更新
     - 書き込み完了後、`gh-cli-best-practices` の VERIFY操作（Section 5-8）に従って内容を検証すること。
 6. REQ 内容更新（パターンBの場合）:
    - Issue本文からREQ番号を抽出（`REQ-{NNNN}` パターンを検索）
    - 各REQファイルを読み込み、実装結果を要件に反映:
      - 実装で要件が満たされたことを確認
      - 実装によって要件内容が変化した場合、該当セクション（目的/要件/適用範囲）をUPDATE
      - 実装で新たな要件が発生した場合、要件テーブルに行をAPPEND
    - frontmatter `updated` フィールドを現在日時に更新
    - `docs/requirements/README.md` テーブルの該当行のタイトル（変更時のみ）を更新
 7a. Issueクローズ（`gh issue close --reason completed`）
 7b. ブランチ・worktree削除 → `git-worktree` スキルの「worktree削除手順」に従って以下を実行:
...
11. ドラフトの `## draft-meta` セクションの `status` を `closed` に更新する（ドラフトが存在する場合のみ）
12. 完了報告 → `issue-guide-reports` の完了報告フォーマット
...
- Step 7b のブランチ・worktree削除（ローカル+リモート）は必ず実行し、成否を確認すること。削除失敗時は警告表示して停止
 - Step 7c の `git pull` は必ず実行すること。pull失敗時は自動解決せずエラー報告して停止
